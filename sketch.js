...
function draw() {
    ...
    if (
        ...
        if (jelly.pos.x < block.pos.x) {
            jelly.pos.x = block.pos.x - (jelly.radius + block.size.x * 0.5);
    ...
    if (
        ...
        if (jelly.pos.y < block.pos.y) {
            jelly.pos.y = block.pos.y - (jelly.radius + block.size.y * 0.5);
            // block.pos.x-block.size.x*.5 is the left side of the box
            // subtract jelly.radius for the center of the jellyfish
        }
        // hit from below
    ...
    pop();
    // tentical.render();
    popup("Jellyfish Demo:\nMovement: Arrow keys or WASD",5)
}
...
function keyInput() {
    if (keyIsDown(RIGHT_ARROW) || key === "d") {
        move_animation(createVector(-1, 0));
    }
    if (keyIsDown(LEFT_ARROW) || key === "a") {
        move_animation(createVector(1, 0));
    }
    if (keyIsDown(DOWN_ARROW) || key === "s") {
        move_animation(createVector(0, -1));
    }
